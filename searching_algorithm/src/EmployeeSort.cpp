...
void EmployeeSort::seqSearch()
{
    ...
    int found = 0;
    int index = 0;
    while(!found && index < empCount)
    {
        if (emps[index].getEmpID() == id)
        {
            found = 1;
            break;
        }
...
// binary search
void EmployeeSort::binarySearch()
{
    if (!sorted)
    {
        return;
    ...
    {
        mid = (high + low) / 2;
        if (emps[mid].getEmpID() == id)
        {
            found = 1;
        } else if (emps[mid].getEmpID() > id)
        {
            high = mid-1;
        }else {
            low = mid+1;
...
void EmployeeSort::printEmp(int i)
{
    ConsoleColor C;
    C.LPurple();
